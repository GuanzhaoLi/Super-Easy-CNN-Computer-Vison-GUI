...
class LabelTool():
    ...
    def __init__(self, master):
        ...
        self.imageDir = ''
        self.imageList= []
        self.pureImageList=[] #image without dir path
        self.egDir = ''
        self.egList = []
        self.outDir = ''
        self.cur = 0
    ...
    def loadDir(self, dbg = False):
        ...
        self.pureImageList=os.listdir(self.imageDir)
    ...
    def loadImage(self):
        ...
        self.labelImageName = os.path.join(self.imageDir, os.path.basename(imagepath))
        bbox_cnt = 0
    ...
    def saveImage(self):
        with open(self.labelfilename, 'w') as f:
            f.write(str(self.labelImageName)+" ")
            for bbox,bboxcls in zip(self.bboxList,self.bboxListCls):
                xmin,ymin,xmax,ymax = bbox
                b = (int(float(xmin)), int(float(ymin)), int(float(xmax)) , int(float(ymax)))
                #bb = self.convert((self.curimg_w,self.curimg_h), b)
                f.write(" " + ",".join([str(a) for a in b]) + ","+str(bboxcls))
    ...
    def mouseClick(self, event):
        ...
        if self.STATE['click'] == 0:
            ...
            #with open("test.txt", "a+") as filehandle:
                #filehandle.write(str(self.labelfilename)+" "+ str(x1)+" "+str(y1)+" "+str(x2)+" "+str(y2))
    ...
    def nextImage(self, event = None):
        ...
        if self.cur < self.total:
            ...
            with open ("train.txt", "w+") as filehandle:
                for item in self.pureImageList:
                    with open(os.path.join(self.outDir,item), 'r') as f:
